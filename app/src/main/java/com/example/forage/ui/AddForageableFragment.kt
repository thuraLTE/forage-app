...
import com.example.forage.viewmodel.ForageableViewModel
import com.example.forage.viewmodel.ForageableViewModelFactory
...
class AddForageableFragment : Fragment() {
    ...
    private lateinit var currentForageable: Forageable
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (id > 0) {
            (requireActivity() as MainActivity).supportActionBar?.setTitle(R.string.update_forage)
            // TODO: Observe a Forageable that is retrieved by id, set the forageable variable,
            //  and call the bindForageable method
            viewModel.selectedForageable(id).observe(viewLifecycleOwner) { forageable ->
                if (forageable != null) {
                    currentForageable = forageable
                    bindForageable(currentForageable)
                }
            }

            binding.deleteBtn.visibility = View.VISIBLE
            binding.deleteBtn.setOnClickListener {
                deleteForageable(currentForageable)
            }
        } else {
            (requireActivity() as MainActivity).supportActionBar?.setTitle(R.string.add_forage)
